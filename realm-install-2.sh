...
REPO_URL="https://host.wxgwxha.eu.org/https://raw.githubusercontent.com/haoxiang2004/realm-install/main/realm-install.sh"
...
auto_install() {
    if [[ ! -f "$REALM_BIN" || ! -f "$WEB_PY" ]]; then
        ...
        local latest_ver=$(curl -s https://host.wxgwxha.eu.org/https://api.github.com/repos/zhboner/realm/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        [[ -z "$latest_ver" ]] && latest_ver="v2.6.0"
        local dl_url="https://host.wxgwxha.eu.org/https://github.com/zhboner/realm/releases/download/${latest_ver}/realm-${realm_arch}.tar.gz"
